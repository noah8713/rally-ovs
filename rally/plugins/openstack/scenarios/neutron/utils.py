...
class NeutronScenario(scenario.OpenStackScenario):
    ...
    address_sets_path = "/address_sets"
    address_set_path = "/address_sets/%s"
    address_set_addrs_path = "/address_sets/%s/addrs"
    address_set_addr_path = "/address_sets/%s/addrs/%s"
    ...
    def _create_security_group_rule(self, **security_group_rule_create_args):
    ...
    def _delete_security_group_rule(self, security_group_rule):
        """Delete Neutron security group rule.
        """
        return self.clients("neutron").delete_security_group_rule(
    ...
    def _list_security_group_rules(self, **kwargs):
        """Return list of Neutron security group rules."""
        return self.clients("neutron").list_security_group_rules(**kwargs)


    @atomic.action_timer("neutron.create_address_set")
    def _create_address_set(self, **address_set_create_args):
        """Create Neutron address-set.

        param: address_set_create_args: dict, POST /v2.0/address-sets
                                          request options
        return: dict, neutron address-set
        """
        address_set_create_args["name"] = self.generate_random_name()
        return self.clients("neutron").post(self.address_sets_path,
            {"address_set": address_set_create_args})

    @atomic.action_timer("neutron.delete_address_set")
    def _delete_address_set(self, address_set):
        """Delete Neutron address set.

        param: address_set: dict, neutron address_set
        """
        return self.clients("neutron").delete(self.address_set_path %
                                              address_set["address_set"]["id"])

    @atomic.action_timer("neutron.list_address_sets")
    def _list_address_sets(self, **kwargs):
        """Return list of Neutron address set."""

        return self.clients("neutron").list('address_sets',
                                            self.address_sets_path,
                                            retrieve_all=True,
                                            **kwargs)

    @atomic.action_timer("neutron.update_address_set")
    def _update_address_set(self, address_set,
                               **address_set_update_args):
        """Update Neutron address-set.

        param: address_set: dict, neutron address_set
        param: address_set_update_args: dict, POST /v2.0/address-sets
                                           update options
        return: dict, updated neutron address-set
        """
        self._warn_about_deprecated_name_kwarg(address_set,
                                               address_set_update_args)
        body = {"address_set": address_set_update_args}
        return self.clients("neutron").put(
                    self.address_set_path %address_set["address_set"]["id"],
                    body)

    @atomic.action_timer("neutron.create_address_set_addr")
    def _create_address_set_addrs(self, address_set_id, addrs):
        """Create Neutron address in address-set.
        """
        ret = []
        for addr in addrs:
            a = self.clients("neutron").post(
                        self.address_set_addrs_path % address_set_id,
                        addr)
            ret.append(a)

        return ret

    @atomic.action_timer("neutron.create_address_set_addr_bulk")
    def _create_address_set_addr_bulk(self, address_set_id, addr_create_body):
        """Create Neutron address in address-set.
        """
        return self.clients("neutron").post(
            self.address_set_addrs_path % address_set_id,
            {"addrs": addr_create_body})


    @atomic.action_timer("neutron.delete_address_set")
    def _delete_address_set_addr(self, address_set_id, addr):
        """Delete Neutron address in address set.
        """
        return self.clients("neutron").delete(
                    self.address_set_addr_path % (address_set_id,
                                                  addr["addrs"]["id"]))
